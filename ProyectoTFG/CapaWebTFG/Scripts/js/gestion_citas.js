    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (parsedResponse.success) {
                ...
                table += '<th>ID</th><th>Motivo</th><th>Fecha</th><th>Persona_ID</th><th>Estado</th><th>Acciones</th>';
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (parsedResponse.success) {
                ...
                $('#editFecha').val(fecha);
                $('#editPersona_Id').val(cita.Persona_Id); 
...
function guardarCita() {
    ...
    // Validar que todos los campos requeridos estén llenos
    if (!citas_Id || !motivo || !fecha || !persona_Id || !estado) {
        alert("Por favor, completa todos los campos requeridos.");
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ citas_Id: citas_Id, motivo: motivo, fecha: fecha, persona_Id: persona_Id, estado: estado }), // Asegúrate de que los nombres coincidan
        success: function (response) {
            let parsedResponse = JSON.parse(response.d);
            if (parsedResponse.success) {
